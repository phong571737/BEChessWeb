...
InferredMove: TypeAlias = chess.Move | tuple[chess.Move, ...] | None
...
def _piece_board(fen: str) -> chess.Board:
    return chess.Board(f"{fen.split(maxsplit=1)[0]} w - - 0 1")


def _boards_for_both_turns(fen: str) -> tuple[chess.Board, chess.Board]:
    position = fen.split(maxsplit=1)[0]
    return (
        chess.Board(f"{position} w - - 0 1"),
        chess.Board(f"{position} b - - 0 1"),
    )


def _legal_in_either_turn(
    move: chess.Move,
    boards: tuple[chess.Board, chess.Board],
) -> bool:
    ...
    for from_square in changed:
        ...
        for to_square in changed:
            ...
            if (
                ...
                and destination_piece.color == moving_piece.color
            ):
                continue
            arrived_piece = after.piece_at(to_square)
            if arrived_piece is None or not _same_moving_piece(
                moving_piece, arrived_piece
            ):
                continue
            promotion = (
            ...
            candidates.append(
                chess.Move(from_square, to_square, promotion=promotion)
            )
...
def _same_moving_piece(
    before_piece: chess.Piece,
    after_piece: chess.Piece,
) -> bool:
    if before_piece.color != after_piece.color:
        return False
    if before_piece.piece_type == after_piece.piece_type:
        return True
    return (
        before_piece.piece_type == chess.PAWN
        and after_piece.piece_type
