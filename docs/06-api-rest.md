...
Rebuilds a saved review record through the Python `recover_service` using
`fenHistoryEdited` when that administrator correction exists, otherwise the
persisted ordered raw `fenHistory`, together with `initialFen`. The endpoint
returns `rawFenHistory`, `fenHistoryEdited`, `preferredFenHistory`, and
`fenSource` (`edited` or `raw`) alongside recovery branches and step metadata.
Recovery output is not persisted as a third FEN field and never overwrites the
raw electronic-board sequence.
...
- `{"command":"restart_game"}` resets the active game in place and retains its `gameID`. A physical long-press sends `{"command":"resign"}` without a side; the backend evaluates the current game when possible, otherwise records `Result: "*"`, `historyStatus: "finished"`, and `outcomeStatus: "unconfirmed"`, then creates the next waiting game and publishes `{"command":"restart_game"}` to the board. The normal `historyStatus: "active"` / “In progress” state is reserved for games that have moves but have not been resigned, drawn, or restarted.
...
### `PUT /games/history/:id/traces`

Administrator-only. Updates the editable review traces without modifying raw
electronic-board FEN snapshots or `fenHistoryEdited`. The body may contain
`{ "pgn": "..." }`, `{ "uciHistory": ["e2e4", "e7e5"] }`, or both. PGN is
limited to 200,000 characters and UCI history to 2,000 short string tokens.
Saving either trace clears cached Stockfish analysis because the analyzed move
sequence may have changed. The response returns the saved `pgn` and
`uciHistory` values.

### `POST /games/history/:id/fens`

Administrator-only. Adds an administrator-supplied FEN snapshot to the
separate `fenHistoryEdited` sequence. If no edited sequence exists yet, the
server clones the raw `fenHistory` first; raw electronic-board data is never
changed. The JSON body is `{ "fen": "..." }` to append, or
`{ "fen": "...", "afterIndex": 12 }` to insert directly after the zero-based
snapshot at index `12`. This is the API used by the review screen's `+` action
to duplicate a selected FEN position before editing it on the board. Chess
legality is intentionally not checked. The response is
`{ success: true, fenHistoryEdited: [...] }`.

### `PUT /games/history/:id/fens`

Administrator-only. Atomically replaces the complete `fenHistoryEdited`
sequence from
`{ "fenHistory": ["...", "..."] }`. The request must contain between 1 and
1,000 non-empty string values. Chess legality is not checked before the atomic
write; a non-string or empty row returns `400` with its zero-based `index`, while
a concurrent edit returns `409`. PGN, UCI history, move timing, and result remain unchanged,
and saved Stockfish analysis is cleared because it belongs to the old sequence.

### `PUT /games/history/:id/fens/:index`

Administrator-only. Replaces the zero-based snapshot in `fenHistoryEdited` at `index` with the
non-empty string in `{ "fen": "..." }` without checking chess legality. PGN,
UCI history, timing, and result are
...
Administrator-only. Removes the zero-based snapshot at `index` from
`fenHistoryEdited` (cloning raw history first when this is the first edit).
The operation deliberately leaves raw `fenHistory`, PGN, UCI history,
...
`PUT /games/history/:id/result` lets an administrator correct an unfinished or completed history result with `1-0`, `0-1`, or `1/2-1/2`. It replaces the stored result, confirms the outcome, and keeps the PGN `Result` header and terminal movetext marker consistent without changing the recorded moves.
