...
sequenceDiagram
  ...
  participant Realtime as Socket.IO + MQTT

  App->>Config: Load configuration
  App->>HTTP: Create app and routes
  App->>DB: Connect and ping
  DB-->>App: Ready
  App->>DB: Restore active games
  App->>DB: Sync optional accounts
  App->>Realtime: Initialize realtime services
  App-->>HTTP: Start listener
```
