...
History deletion and FEN correction are administrator-only at both layers. The frontend hides and disables moving records to trash, viewing/restoring trash, permanent deletion, empty-trash actions, and adding, editing, or deleting individual FEN snapshots for standard users. Every corresponding backend route also uses `requireAdmin`; a valid `user` JWT therefore receives HTTP `403` even if the endpoint is called manually. FEN input is validated with the chess library, and every FEN mutation uses an exact-array write guard to prevent concurrent corrections from overwriting each other.
