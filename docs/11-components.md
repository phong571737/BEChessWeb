...
Shows engine or evaluation metadata when present in the board state. Its responsive breakpoint matches the full board's two-column layout: from `sm` upward it stays vertically beside the board; only phone-width layouts render the horizontal bar below the board. Move Review reuses the component with the saved evaluation for its selected ply, but owns a separate `history-show-evaluation` preference so hiding it never changes the live Board preference.
...
Renders the review-page Stockfish summary from the analysis for the source currently selected by that browser tab. The Review page passes the live `analysisMoves` array into the component; persisted `game.analysis.moves` is only an initial fallback for older records or a previously stored result. It counts Brilliant, Best, Excellent, Good, Inaccuracy, Mistake, and Blunder labels for White and Black from each ply number, then compares both sides in a grouped vertical bar chart. It does not reparse PGN/FEN data or present capture/check counters as engine classifications. When the selected source has no completed engine rows, it renders a safe empty state rather than zero-filled misleading statistics.

### `move-analysis-panel`

Runs browser Stockfish automatically for the History source selected by the current viewer. For the original source it uses `fenHistoryEdited` when an administrator has corrected the timeline, otherwise the immutable electronic-board `fenHistory`; a recovered branch supplies its own recovered FEN sequence. The panel reports each newly produced analysis array to `PGNReviewContent`, which lifts it to the Review page so the board annotation, evaluation bar, advantage chart, accuracy cards, and `match-analysis` component all consume the same source-local result. The callback retains the legacy name `onAnalysisSaved`, but database persistence is not required for the live UI update.

Switching sources clears the previous source's rows before a new search starts, preventing branch A statistics from appearing over branch B. Classifications use standard NAG-style marks: `?!` for an inaccuracy, `?` for a mistake, and `??` for a blunder; positive classifications use `!!`, `!`, and `!?`. The colored mark is overlaid on the move's destination square and is not repeated in the move list. The list instead shows thinking time when available. A transition that cannot be reconstructed from consecutive FEN snapshots is rendered with the localized **Unavailable** / **Không khả dụng** label without blocking later positions. Null engine scores are omitted from the advantage chart instead of being rendered as a false `0.0` evaluation.
