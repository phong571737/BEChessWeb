...
The current application release is `v1.1.3-change3`. The root package, frontend package, and `frontend/lib/app-version.ts` must use the same version.
...
```powershell
git tag -a v1.1.3-change3 -m "Release v1.1.3-change3"
git push origin v1.1.3-change3
```
