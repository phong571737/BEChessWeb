...
Every vertical evaluation bar is placed in the same flex row as its chessboard
and receives the measured board width as its height. This keeps the bar aligned
with the board even when the review editor adds spare-piece strips above and
below the board. Narrow layouts use the horizontal evaluation bar where the
board/control layout no longer has room for a vertical bar.
