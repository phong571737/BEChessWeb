...
The paste page accepts either UCI tokens or one FEN position per line. UCI input is parsed locally; FEN timelines are sent to `POST /games/recover`, which uses the internal `recover-service` algorithm and returns a PGN plus recovery status. If the sidecar is unavailable, the backend uses the local unchecked renderer and marks unresolved transitions as `x`. Its layout follows the application page standard: a compact bordered page header, a centered `max-w-7xl` content area, and shared background/card/border/primary tokens. The input and preview cards stack through phone, tablet, and standard desktop widths, then become matched-height columns on desktop. Every container, grid item, and card is explicitly shrinkable (`min-w-0`) so long controls cannot force horizontal overflow. The import, preview, copy, download, and branch-selection behavior remains unchanged.
