...
Administrators can append, insert, edit, or delete individual entries in the FEN list, or open the bulk editor to replace the complete sequence by pasting one FEN per line. Numbered lines such as `1. <FEN>` are accepted. The `+` action beside each base-source FEN inserts a duplicate immediately below that row; the copied position is then editable directly on the standard `ChessBoardView`. Admins can drag an existing piece, drag a spare piece from the black/white piece strips, click a spare piece and then a square, or right-click a square to remove its piece. Saving creates/updates `fenHistoryEdited`; raw electronic-board snapshots remain unchanged. The backend trims each value but intentionally does not validate FEN structure or chess legality before saving, allowing damaged physical-board snapshots to remain available to the recovery service. Only non-string or empty entries are rejected. The replacement is atomic, and concurrent edits are rejected instead of overwriting another administrator's correction. Every FEN mutation clears stale saved Stockfish analysis and refreshes recovery from the corrected list. PGN, UCI history, move timing, and game result remain unchanged.
...
The live Back/Next controls prioritize the persisted `initialFen` plus `fenHistory` snapshots, and use a parsed PGN timeline only when no FEN history exists. The hook refreshes that timeline once per `gameID`; move events append snapshots, while restore events that omit history preserve the already loaded sequence instead of erasing navigation state.
...
The review page is for deeper historical analysis and branch navigation. It opens on the original source. That source uses `fenHistoryEdited` when an administrator has corrected the timeline and otherwise uses the immutable FEN snapshots received from the electronic board. Every recovered candidate is an independent branch with its own FEN sequence, notation, board positions, and browser-local Stockfish result.

Stockfish analysis starts automatically for the selected source and does not need to be saved to MongoDB before the UI can use it. `PGNReviewContent` builds the selected source as `analysisGame`, `MoveAnalysisPanel` produces the move-analysis array, and the Review page lifts that array into `analysisMoves`. The same array is then passed to Match Analysis, the accuracy cards, the advantage chart, the selected-ply details, the evaluation bar, and the board annotation. Persisted `game.analysis.moves` is an initial fallback only. When the viewer switches source, the previous array is cleared before the new source is analyzed, so statistics from different branches cannot be mixed. Different viewers may select and analyze different branches without changing each other's display.

Match Analysis summarizes Brilliant, Best, Excellent, Good, Inaccuracy, Mistake, and Blunder classifications by side. It does not label capture/check counters as engine analysis. A source with no completed engine rows displays an empty state instead of zero-filled broken charts.

Move Review requests `GET /games/history/:id/recovered-pgn`. The source header uses one compact native selector with a source count rather than rendering every recovery branch as a button; even a large candidate set therefore cannot displace the move list, PGN, or navigation controls. The full PGN presentation pairs both plies beneath one conventional move number (`1. e4 e5`). If a recovery step has multiple compatible candidates, the move row shows a branch control and the user can select a candidate. Recovery rows annotate generated and collapsed observations with text such as `f5 (PADDED)`, `e4 (Assumed)`, and `e4 (duplicate clean 3 FEN)`. FEN-only review entries render on their own row. A transition that cannot be reconstructed from consecutive FEN snapshots is shown as a localized unavailable row, while scoreless points are omitted from the advantage chart.

The FEN list keeps its native disclosure icon and same-row localized Copy FEN and Download as text file actions. Administrators can maintain `fenHistoryEdited` only after the game is complete; the original electronic-board `fenHistory` remains unchanged. The review board keeps the existing chessboard skin while exposing the black spare-piece strip above the board and the white strip below it. The vertical evaluation bar is rendered in the same board row with exactly the board's rendered height. On narrow layouts, the evaluation/suggestion/annotation controls are grouped under one icon menu so the spare-piece strip remains centered over the board. Download calls `GET /games/history/:id/fen-text`, which returns a recovery-service-compatible numbered text timeline with the record ID and starting FEN headers. All recovery-related HTTP routes are defined in `backend/src/routes/recover.router.ts`; administrator FEN correction routes remain with the protected history routes in `game.router.ts`. The login form includes an accessible icon-only control to show or hide the password. Empty or partial histories display safe empty states.
