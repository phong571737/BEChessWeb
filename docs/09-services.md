...
`recover_service` is an optional Python/FastAPI Docker sidecar. It exposes `POST /recover` on the internal Compose network at `http://recover-service:8000`. The backend sends ordered `fenHistory`, the starting FEN, and PGN headers when finalizing a game. The sidecar compares consecutive piece-placement snapshots, filters ambiguous candidates by the expected moving color, detects castling as one king move, explores compatible legal branches, and returns `originalPgn` plus recovery metadata. Pairwise notation remains available for partially legal physical-board traces: an illegal-but-unambiguous transition uses unchecked SAN-style notation, while only a genuinely ambiguous transition is represented by `X`. The history review endpoint (`GET /games/history/:id/recovered-pgn`) calls the same sidecar so the Move Review list and PGN notation use one consistent recovery algorithm. FEN-backed Move Review has no browser-side PGN reconstruction path: if the sidecar is unavailable, the UI reports that condition and does not mix persisted UCI or a separate frontend algorithm into the notation. Game finalization retains its backend fallback so a sidecar outage cannot prevent a resignation from being saved.

The sidecar is not exposed publicly and is enabled by `RECOVER_SERVICE_URL`, which must be supplied through the ignored deployment `.env`. `RECOVERY_TIMEOUT_MS` controls how long the backend waits for one recovery request and defaults to 60 seconds; longer histories with ambiguous transitions can require most of that budget because branch exploration grows quickly. It is built by `recover_service/Dockerfile` and started by `docker-compose.yml` before the backend. The backend client uses Node's native HTTP/HTTPS modules because the Stockfish Lite Node compatibility layer clears the process-wide `fetch` function during WASM initialization. If the sidecar is unavailable, the backend falls back to the local unchecked renderer and marks unresolved transitions as `x`; this keeps Paste and game finalization available without affecting MQTT, clocks, or normal move processing.
...
- subscribe to `chess/+/command` and process lifecycle commands. `restart_game` resets the active board game in place; physical `restart_game_esp` first runs the backend Stockfish result policy and finalizes/creates the next game when the result is clear, otherwise it finalizes the old history as unfinished/unconfirmed and creates the next game. Restart is intentionally handled only on the command topic, not as a status value. `resign` accepts `{"command":"resign","side":"white"|"black"}` and `draw` accepts `{"command":"draw"}`. These commands resolve the active game by board, use the same atomic resignation service as the web API, emit the old game result to its room, and create/announce the next waiting game so an open board page and the home board card update consistently.
