...
- REST and app MQTT restart commands use the same in-place game-action service and retain the same `gameID`. A physical `restart_game_esp` command is different: when the game has at least two plies, the backend Stockfish service evaluates the last FEN, finalizes the old game with the inferred loser when the advantage reaches ±150 cp (or a mate score), and creates the next waiting game for the board; an unavailable/uncertain evaluation finalizes the old session as `historyStatus: "finished"`, `outcomeStatus: "unconfirmed"`, and `Result: "*"`, then creates the next waiting game. This is displayed separately from an active/in-progress game. After a successful web REST restart, the backend also publishes the non-retained `{"command":"restart_game"}` command to `chess/<boardID>/command`, so the ESP32 resets its local shadow board and init-check state. The backend marks this outbound message with an internal origin so its own MQTT subscription does not execute the reset twice. The legacy protected Socket.IO `restart` listener emits a room refresh only and is not the persistent command path used by the current web UI.
