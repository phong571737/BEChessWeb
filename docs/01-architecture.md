...
flowchart LR
    ...
    API --> DB
    ESP32 -->|HTTPS: boards + moves| Proxy
    ESP32 <-->|status and commands| MQTT
    API <-->|board events| MQTT
```
...
| Realtime backend | Socket.IO | Emits global move snapshots plus room-scoped game lifecycle/clock events. |
...
| Browser engine | Stockfish WebAssembly worker | Optional live evaluation and authenticated review analysis; it never decides server game state. |
...
```mermaid
flowchart TD
    Start([Start]) --> Config[Load environment]
    Config --> HTTP[Create Express and HTTP server]
    HTTP --> DB[Connect MongoDB]
    DB --> Restore[Restore active games]
    Restore --> Accounts[Sync optional accounts]
    Accounts --> Realtime[Initialize Socket.IO and MQTT]
    Realtime --> Listen([Listen on PORT])
```
...
sequenceDiagram
    ...
    participant API as Move service
    ...
    ESP->>API: Submit move
    API->>Runtime: Validate and apply
    Runtime->>DB: Save game and traces
    API->>DB: Update history snapshot
    API->>IO: Broadcast game update
    IO-->>UI: Room event
    UI->>UI: Update board and clock
```

Board status and lifecycle commands use MQTT topics (`chess/<boardID>/status` and `chess/<boardID>/command`), while physical move payloads are submitted through the move HTTP API. The status topic is reserved for connectivity (`online`/`offline`); lifecycle commands are accepted only on the command topic. `restart_game` resets the active game in place. A physical long-press sends `{"command":"resign"}` without a side; the backend finalizes the old game by evaluation when possible (or marks the outcome unconfirmed), creates the next waiting game, then publishes `{"command":"restart_game"}` back to the board. Explicit commands can resign a side (`{"command":"resign","side":"white"|"black"}`) or agree a draw (`{"command":"draw"}`). Resign/draw uses the same atomic service as the web action, emits the terminal result to the old game room, and publishes the next waiting game so clients can leave the finished game state and retain the physical-board mapping. For NFC/device snapshots, the backend can load a board FEN with validation skipped, keep raw UCI/FEN traces, and build custom notation. This preserves the physical-board record even if the device sequence cannot be replayed as a fully legal `chess.js` game.

### Delayed spectator delivery

Move delivery has two audiences. Authenticated administrators receive the authoritative `esp_move` immediately. Public viewers receive a delayed, sequential stream controlled by `spectatorDelayMs` in the broadcast settings; the default is `0`, so no delay is applied until an administrator enables it. The delay service stores pending events in `spectator_events` and the latest public state in `public_game_snapshots`.

The delay is applied per accepted move, preserving the original move spacing. It is not a timer that releases the whole queue at once. On service restart, pending events are rescheduled with the same remaining offset so the public stream does not jump ahead. A public reload or Socket.IO reconnect hydrates from the delayed public snapshot, so reload cannot bypass the configured delay. Active-game snapshots are deduplicated by `boardID` before they are sent to the home dashboard.

### Restart, resign, and recovery

- REST and `restart_game` MQTT restart commands use the same in-place game-action service and retain the same `gameID`. A physical long-press is represented by a side-less `resign` command: when the game has at least two plies, the backend Stockfish service evaluates the last FEN, finalizes the old game with the inferred loser when the advantage reaches ±150 cp (or a mate score), and creates the next waiting game for the board; an unavailable/uncertain evaluation finalizes the old session as `historyStatus: "finished"`, `outcomeStatus: "unconfirmed"`, and `Result: "*"`, then creates the next waiting game. After finalization the backend publishes the non-retained `{"command":"restart_game"}` command to `chess/<boardID>/command`, so the ESP32 resets its local shadow board and init-check state. The backend marks this outbound message with an internal origin so its own MQTT subscription does not execute the reset twice. The legacy protected Socket.IO `restart` listener emits a room refresh only and is not the persistent command path used by the current web UI.
...
Move Review derives replay and statistics from the selected FEN source. Stockfish analysis uses adjacent FEN snapshots only: edited FEN for the base source when present, raw electronic-board FEN otherwise, or the selected recovered branch. UCI and PGN are not substituted into this analysis path. A custom or malformed physical-board transition is stored as an unavailable analysis row rather than being invented as an equal engine score or stopping the complete history analysis.
...
The validated Archify artifacts for this architecture are available as standalone HTML files:

- [Runtime architecture diagram](diagrams/ttlab-chess-architecture.html)
- [Live move sequence diagram](diagrams/ttlab-chess-live-sequence.html)

The JSON specifications beside these files are the editable Archify sources. Mermaid blocks above remain as lightweight inline references for GitHub readers.
